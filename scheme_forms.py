...
def do_and_form(expressions, env):
    ...
    last_eval = True
    while current != nil:
        last_eval = scheme_eval(current.first, env)
        if is_scheme_false(last_eval):
            return False
...
def make_let_frame(bindings, env):
    ...
    names = bindings.map(lambda arg: arg.first)
    validate_formals(names)
    values = bindings.map(lambda arg: scheme_eval(arg.rest.first, env))
    # END PROBLEM 14
